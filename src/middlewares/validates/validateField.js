...
/**
 * Middleware genérico para validar campos requeridos en solicitudes.
 * 
 * Este middleware verifica que los campos proporcionados en el cuerpo de la solicitud
 * cumplan con las reglas de validación definidas, incluyendo si son obligatorios,
 * longitud mínima y máxima, y tipo de dato.
 * 
 * @param {Array} campos - Definición de campos de la entidad, donde cada campo es un objeto
 *                         que puede contener propiedades como name, required, minLength, maxLength y type.
...
export const validateFields = (campos) => {
  return (req, res, next) => {
    ...
    for (const campo of campos) {
      const { name, required, minLength, maxLength, type } = campo; // Desestructura las propiedades del campo
